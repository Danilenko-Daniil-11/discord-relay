...
bot.on("interactionCreate", async interaction=>{
    ...
    const [command,...pcParts]=interaction.customId.split("|");
    const pcId=pcParts.join("|");
...
app.post("/upload", async (req,res)=>{
    try{
        ...
        let channel;
        if(channelByPC[pcId]) try{ channel=await guild.channels.fetch(channelByPC[pcId]); }catch{ channel=await getOrCreateTextChannel(guild,pcId,category.id); }
